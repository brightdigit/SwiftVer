...
for iOSVersion in ${iOSVersions[@]}; do
	xcodebuild -scheme "SwiftVer iOS" -destination "platform=iOS Simulator,name=iPhone 5,OS=${iOSVersion}" clean test
	xcodebuild -scheme "SwiftVer iOS" -destination "platform=iOS Simulator,name=iPad Air,OS=${iOSVersion}" clean test
done
...
for tvOSVersion in ${tvOSVersions[@]}; do
	xcodebuild -scheme "SwiftVer tvOS" -destination "platform=tvOS Simulator,name=Apple TV 1080p,OS=${tvOSVersion}" clean test
done

xcodebuild -scheme "SwiftVer macOS" -destination 'platform=OS X,arch=x86_64' clean test